...
exports.register = function(res, mail, pwd, name, fname) {
    db.execute('INSERT INTO `user` (email, password, name, firstname) VALUES (?, ?, ?, ?)', [mail, pwd, name, fname], function(err, results, fields) {
        const token = jwt.sign({email:mail, password:pwd}, 'SECRET');
        res.status(200).json({token});
    })
}

exports.check_account_name = function(res, name, callback) {
    db.execute('SELECT * FROM `user` WHERE name = ?', [name], function(err, results, fields) {
        if (results.length > 0)
            callback(84);
        else
            callback(0);
...
exports.update_user_by_id = function(res, id, email, pwd, name, fname) {
    db.execute('UPDATE `user` SET email = ?, password = ?, name = ?, firstname = ? WHERE id = ?', [email, pwd, name, fname, id], function(err, results, fields) {
